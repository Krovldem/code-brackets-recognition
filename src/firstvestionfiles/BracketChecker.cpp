...
BracketChecker::BracketChecker(const std::string& errorFile) : logger(errorFile) {}

bool BracketChecker::checkFile(const std::string& filename) {
    std::ifstream file(filename);
    if (!file.is_open()) {
        logger.logError(0, 0, "Cannot open file");
        return false;
    }
    ...
    if (inSpecialComment) {
        logger.logError(commentStartLine, commentStartPos, "Unclosed special comment");
    }
...
void BracketChecker::processCharacter(char currentChar, int lineNumber, int position, const std::string& line) {
    if (currentChar == '(' || currentChar == '[' || currentChar == '{') {
        ...
        if (stack.isEmpty()) {
            logger.logError(lineNumber, position, "Unexpected closing bracket");
        } else {
            ...
            if ((topBracket == '(' && currentChar == ')') ||
                ...
                logger.logError(lineNumber, position, "Mismatched bracket");
            }
        }
    }
}
...
void BracketChecker::validateRemainingBrackets() {
    while (!stack.isEmpty()) {
        StackNode* topNode = stack.peek();
        logger.logError(topNode->lineNumber, topNode->position, "Unclosed bracket");
        stack.pop();
    }
}
...
std::vector<ErrorInfo> BracketChecker::getErrors() const {
